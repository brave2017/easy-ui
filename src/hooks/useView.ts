      ...
      return new Promise((resolve, reject) => {
        ...
        ElMessageBox.confirm("确定进行[删除]操作?", "提示", {
          ...
          type: "warning"
            ...
            baseService.post(`${state.deleteURL}`,
              state.deleteIsBatch ? {ids: state.dataListSelections && state.dataListSelections.length > 0 ? state.dataListSelections.map((item: IObject) => state.deleteIsBatchKey && item[state.deleteIsBatchKey]) : [id]} : {id : id}
            ).then((res) => {
                ElMessage.success({
                  message: "成功",
                  duration: 500,
